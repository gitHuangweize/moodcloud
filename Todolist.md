...
- [ ] **第 1 晚（2h）**：云图抽样 + 换一批
  - 产出：渲染时仅抽样 N 条（如 200）；加“换一批”按钮
  - 自测：数据库数据多时仍流畅
- [ ] **第 2 晚（2h）**：错误处理/空态 + 基础打磨
...
- [ ] **第 4 周**：点赞去重（likes 表）+ 删除自己的内容 + 个人主页增强
  - 产出：同一用户点赞不重复；可取消（可选）；可删除自己的 thought/comment
...
- [ ] **自动分类 ThoughtType（GRUMBLE/INSIGHT/WHISPER）**
...
- [ ] **likes 表（thought_id + user_id 唯一约束）**
...
- [ ] **分页/无限滚动**（thoughts/comments）
  - 验收：大量数据下仍可用，首屏加载稳定。
- [ ] **云图布局优化**（避免重叠、移动端可拖拽/缩放 可选）
