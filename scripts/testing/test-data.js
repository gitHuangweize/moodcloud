...
async function generateTestData(count = 500) {
  ...
  // 这个脚本需要在浏览器控制台中运行，使用 window.supabaseStorageService
  ...
  for (let i = 0; i < count; i++) {
    const thought = {
      ...
      authorId: generateUUID(), // 使用正确的 UUID 格式
      timestamp: Date.now() - Math.random() * 86400000 * 30, // 30天内随机时间
...
console.log('🎯 测试数据脚本已加载！');
console.log('使用方法:');
console.log('  generateTestData(500) - 生成500条测试数据');
